...
import { Solicitation, SolicitationStatus } from '../../../domain/entities/Solicitation.js';
...
export class PrismaSolicitationRepository implements ISolicitationRepository {
  private toEntity(r: any): Solicitation {
    return new Solicitation(
      r.id, r.protocolNumber, r.title, r.description, r.neighborhood,
      r.city, r.uf, r.street, r.requestingUserId, r.solicitationTypeId,
      r.status as SolicitationStatus,
      Array.isArray(r.unsolvedImageUrls) ? r.unsolvedImageUrls : JSON.parse(r.unsolvedImageUrls ?? '[]'),
      r.solvedImageUrls ? (Array.isArray(r.solvedImageUrls) ? r.solvedImageUrls : JSON.parse(r.solvedImageUrls)) : null,
      r.solvedDate, r.solvedCommentary, r.solvedUserId, r.createdAt, r.updatedAt,
    );
  }
  ...
  async update(id: string, data: UpdateSolicitationDTO): Promise<Solicitation> {
    const r = await prisma.solicitation.update({ where: { id }, data: data as any });
    return this.toEntity(r);
  }
  ...
  async list(pagination: PaginationDTO, filters?: { userId?: string; status?: string }): Promise<PaginatedResultDTO<Solicitation>> {
    ...
    const [records, total] = await Promise.all([
      prisma.solicitation.findMany({ skip, take: perPage, where, orderBy: { createdAt: 'desc' } }),
